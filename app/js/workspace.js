...
(function (F) {
  ...
  // ============ 2º PAINEL: folha de referência lado a lado (Etapa 1: navegável + marcas) ============
  (function paneB() {
    ...
    F.toggleSplit = function () {
      ...
      try { resize(); draw(); } catch (e) {}     // canvas A mudou de tamanho → re-renderiza
    };
    (function wire() {
      const go = () => {
        const cvB = document.querySelector('#wsCanvasB'); if (!cvB || cvB._wired) return; cvB._wired = true;
        const sb = document.querySelector('#wsSplit'); if (sb) sb.addEventListener('click', F.toggleSplit);
